...
1. **Wraith · Disrupt → REPLACE.** The current text is a lie (T3 resolves *before* T4, so the engine speeds the enemy up — a buff). Decision: cut Disrupt and replace it. *Constraint:* the regression test "Every class has exactly 20 cards" (`test/regression.mjs:33`) means it must be swapped, not just deleted. **Replacement:** `Flash Bang` (T1 Spell, aim enemy) — `Weaken 3 on one enemy.` — the disruption Disrupt was reaching for, but honest: it actually reduces the enemy's damage this turn. Requires a small engine handler (reuse the existing `turn.outgoing` / Weaken plumbing — no new mechanic) **and a balance sim before it ships**, so it's tracked as its own task, not part of the text-only rewrite.
